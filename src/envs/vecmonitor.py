# https://stable-baselines3.readthedocs.io/en/master/_modules/stable_baselines3/common/vec_env/vec_monitor.html/
...
class VecMonitor(VecEnvWrapper):
    ...
    def __init__(
        ...
        self.fractions_solved = np.zeros(self.num_envs, dtype=np.float32)
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        for i in range(len(dones)):
            self.fractions_solved[i] += infos[i]["solved"]
            if dones[i]:
                info = infos[i].copy()
                episode_return = self.episode_returns[i]
                episode_length = self.episode_lengths[i]
                fraction_solved = self.fractions_solved[i] / self.max_episode_steps
                ep_solved = int(self.fractions_solved[i] > 0)
                episode_info = {
                    "r": episode_return,
                    "l": episode_length,
