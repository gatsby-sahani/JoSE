...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.pre_train_vecenv = (
            VecNormalize.load(pre_train_vecenv_path, env)
            if pre_train_vecenv_path is not None
            else None
        )
        self.pre_train_gradient_steps = pre_train_gradient_steps
    ...
    def load_pre_train_replay_buffer(
        self,
        path: Union[str, pathlib.Path, io.BufferedIOBase],
        truncate_last_traj: bool = True,
    ) -> None:
        ...
        :param truncate_last_traj: When using ``HerReplayBuffer`` with online sampling:
            ...
            If set to ``False``, we assume that we continue the same trajectory (same episode).
        """
        self.pre_train_replay_buffer = load_from_pkl(path, self.verbose).replay_buffer
        assert isinstance(
            self.pre_train_replay_buffer, ReplayBuffer
        ), "The replay buffer must inherit from ReplayBuffer class"
    ...
    def _setup_learn(
        ...
        assert (
            self.env is not None
        ), "You must set the environment before calling _setup_learn()"
        ...
        if (
            ...
            self.action_noise = VectorizedActionNoise(
                self.action_noise, self.env.num_envs
            )
    ...
    def learn(
        ...
        if self.pre_train_updates > 0:
            n_pretrain_updates = 0
            while n_pretrain_updates < self.pre_train_updates:
                self.pre_train(
                    batch_size=self.batch_size,
                    gradient_steps=self.pre_train_gradient_steps,
                )
                n_pretrain_updates += self.pre_train_gradient_steps
                self.logger.record(
                    "pretrain/n_updates", n_pretrain_updates, exclude="tensorboard"
                )
                self.logger.dump(step=n_pretrain_updates - self.pre_train_updates)
