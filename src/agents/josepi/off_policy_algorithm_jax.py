...
from src.agents.josepi.off_policy_algorithm import OffPolicyAlgorithm
...
class OffPolicyAlgorithmJax(OffPolicyAlgorithm):
    def __init__(
        ...
        self.reset_idx = 0
        self.pre_train_buffer_path = pre_train_buffer_path
    ...
    def _excluded_save_params(self) -> list[str]:
        ...
        if hasattr(self, "pre_train_replay_buffer"):
            excluded.append("pre_train_replay_buffer")
    ...
    def _setup_model(self) -> None:
        ...
        if self.pre_train_buffer_path is not None:
            self.load_pre_train_replay_buffer(self.pre_train_buffer_path)
    ...
    def load_pre_train_replay_buffer(
        ...
        super().load_pre_train_replay_buffer(path, truncate_last_traj)
        # Override replay buffer device to be always cpu for conversion to numpy
        assert self.pre_train_replay_buffer is not None
        self.pre_train_replay_buffer.device = get_device("cpu")
