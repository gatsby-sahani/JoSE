...
from sbx.sac.policies import SimbaSACPolicy

from src.agents.play_phase_agent.policies import SACPolicy
...
class SAC(OffPolicyAlgorithmJax):
    ...
    def update_critic(
        ...
        key, noise_key, dropout_key_target, dropout_key_current = jax.random.split(
            key, 4
        )
        ...
        def mse_loss(params: flax.core.FrozenDict, dropout_key: jax.Array) -> jax.Array:
            ...
            current_q_values = qf_state.apply_fn(
                params, observations, actions, rngs={"dropout": dropout_key}
            )
    ...
    def update_actor_and_temperature(
        ...
        ent_coef_state, ent_coef_loss_value = cls.update_temperature(
    ...
    def _train(
        ...
        def one_update(i: int, carry: dict[str, Any]) -> dict[str, Any]:
            ...
            batch_act = jax.lax.dynamic_slice_in_dim(
