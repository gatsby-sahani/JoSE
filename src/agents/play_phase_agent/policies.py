...
class SACPolicy(BaseJaxPolicy):
    ...
    def build(
        ...
        class ActorWithPreprocessor(nn.Module):
            actor_def: nn.Module
            preprocess_fn: Callable[[jnp.ndarray], jnp.ndarray]

            @nn.compact
            def __call__(self, x: jnp.ndarray):
                x = self.preprocess_fn(x)
                return self.actor_def()(x)

        self.actor = ActorWithPreprocessor(
            actor_def=lambda: self.actor_class(
                action_dim=int(np.prod(self.action_space.shape)),
                net_arch=self.net_arch_pi,
                activation_fn=self.activation_fn,
            ),
            preprocess_fn=lambda obs: obs[..., self.pi_and_Q_observations],
        )
        ...
        class ProcessedVectorCritic(nn.Module):
            critic_def: nn.Module  # definition or constructor for VectorCritic
            preprocess_fn: Callable[[jnp.ndarray], jnp.ndarray]

            @nn.compact
            def __call__(self, obs: jnp.ndarray, action: jnp.ndarray):
                obs = self.preprocess_fn(obs)
                return self.critic_def()(obs, action)

        self.qf = ProcessedVectorCritic(
            critic_def=lambda: self.vector_critic_class(
